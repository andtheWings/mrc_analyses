...
referrals <- all_raw %>%
    ...
    filter(
        !is.na(referral_source) | !is.na(primary_cat) | !is.na(disposition)
    ) %>%
    # Coerce variables into un-ordered categories 
    mutate(
        referral_source = as_factor(referral_source),
...
```{r}
referral_cats_by_dispo <- referrals %>%
    group_by(
        primary_cat,
        disposition
    ) %>%
...
Create function for initial collection of people-level data:

```{r}
# Function takes a source file and desired columns as input arguments
collect_people_data <- function(source, columns) {
...
Run custom function defined above to collect admitted client records:

```{r}
mrc_just_people <- collect_people_data(all_raw, people_columns)
```

Collect registration dates and join to the people table:
...
mrc_people_admDate <- all_raw %>%
    ...
    # Only use records associated with an admitted client
    filter(mrc_id %in% mrc_just_people$mrc_id) %>%
    group_by(mrc_id) %>%
...
Collect discharge dates and join to the people table:
...
mrc_people_admDate_dischDate <- all_raw %>%
    ...
    filter(
        mrc_id %in% mrc_just_people$mrc_id &
...
Collect discharge destinations and join to the people table:
...
Some clients needed manual chart review in order to fill in missing data on discharges:
...
Export to parquet file for use by python:

```{r}
write_parquet(mrc_people, "data/mrc_people_model/mrc_people.parquet")
```

Python takes the parquet file and uses the dedupe library to assign cluster ID's to repeat clients.

Import the clustered data back into R:
...
people_summary_tables <- map(
    ...
    .f = ~count_and_percent(clustered_mrc_people, .x)
)
```
