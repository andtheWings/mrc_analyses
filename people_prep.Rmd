...
mrc_people <- people_prep(all_raw, people_columns) %>%
    ...
    summarize(
        ...
        across(
            .cols = mrc_discharge_date:mrc_discharge_dest,
            .fns = ~ last(.x)
        )
    ) %>%
    filter(
    ...
    mutate(
        ...
        discharge_dest_cat = as_factor(
            case_when(
                str_detect(
                    mrc_discharge_dest,
                    "bridge|sro|permanent|apartment|recovery|fillmore"
                ) ~ "Bridge/Stable Housing",
                str_detect(
                    mrc_discharge_dest,
                    "haymarket|rehab|(treatment center)"
                ) ~ "Residential Treatment",
                str_detect(
                    mrc_discharge_dest,
                    "forward|transfer|((?=.*shelter)(?=.*evanston))"
